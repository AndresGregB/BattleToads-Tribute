...
ModuleSceneInferno::ModuleSceneInferno (bool start_enabled):
...
void
ModuleSceneInferno::updatePlatform (movingPlantform * plat)
{
  plat->platformHitbox->hitBox.x =
    plat->position.x +
    plat->amplitude * sin ((double) (SDL_GetTicks () / 10) * 0.01 *
			   plat->speed + plat->variation);

  App->renderer->Blit (objects, plat->platformHitbox->hitBox.x,
		       plat->platformHitbox->hitBox.y, &platformVisual, 1.0f);
