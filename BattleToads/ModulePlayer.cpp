...
Module (start_enabled)
{
  ...
  die.frames.push_back (
			 {
			 549, 118, 38, 34});
  die.speed = 0.1;

  end.frames.push_back (
			 {
			 0, 0, 480, 360});
  end.speed = 0.0f;

  jump.frames.push_back (
			  {
			  415, 167, 46, 25});
...
update_status ModulePlayer::Update ()
{
  ...
  drawLives ();
  if (position.x > 2600
      && !App->collisions->enemiesControler->checkSpawningPoints ())
    {
      lives = 0;
      App->renderer->Blit (endScreen, interfaceX - 160, -30,
			   &(end.GetCurrentFrame ()), 1.0, 2, false);
    }
  return UPDATE_CONTINUE;
}

void
ModulePlayer::playCurrentAnimation ()
{
  ...
  if (AnimStatus == DEAD)
      ...
      if (!onFloor)
	{
      ...
      App->renderer->Blit (graphics, position.x, position.y,
			   &(die.GetCurrentFrame ()), 1.0f);
    }
  ...
  else if (AnimStatus == ATTACK1_RIGHT)
    {
      ...
      if (auxRect.x == 0 && auxRect.y == 0 && auxRect.w == 0
	  && auxRect.h == 0)
	{
...
ModulePlayer::checkInputs ()
{
  ...
  if (AnimStatus == WALKING_LEFT || AnimStatus == JUMPING_LEFT
      || AnimStatus == ATTACK1_LEFT || AnimStatus == ATTACK2_LEFT)
    {
      if (jumping)
	{
      ...
      else
	{
	  AnimStatus = IDLE_LEFT;
	   ...
	   || AnimStatus == ATTACK1_RIGHT || AnimStatus == ATTACK2_RIGHT)
    {
      if (jumping)
	{
  ...
  if (App->input->GetKey (SDL_SCANCODE_D) == KEY_REPEAT
      && App->input->GetKey (SDL_SCANCODE_A) == KEY_REPEAT && !jumping)
    {
  ...
  if (App->input->GetKey (SDL_SCANCODE_W) == KEY_REPEAT
      && coordZ > -3.5 /*Limit to vertical movement */ )
    {
      position.y -= 0.2;
      coordZ -= 0.1;
      floorY = 120 + coordZ * 10;
  ...
  if (App->input->GetKey (SDL_SCANCODE_S) == KEY_REPEAT
      && coordZ < 0 /*Limit to vertical movement */ )
    {
      position.y += 1.8;
      coordZ += 0.1;
      floorY = 120 + coordZ * 10;
  ...
  if (App->input->GetKey (SDL_SCANCODE_D) == KEY_REPEAT
      ...
      if (AnimStatus == IDLE_RIGHT || AnimStatus == WALKING_RIGHT)
	{
	  AnimStatus = IDLE_RIGHT;
	   ...
	   && App->input->GetKey (SDL_SCANCODE_W) == KEY_REPEAT && !jumping)
    {
      if (AnimStatus == IDLE_LEFT || AnimStatus == WALKING_LEFT)
	{
	  AnimStatus = IDLE_LEFT;
...
void
ModulePlayer::drawLives ()
{
  for (int i = 0; i < 5; i++)
    {
      if (App->renderer->cameraLocked)
	{
	  App->renderer->Blit (life, (interfaceX - 120) + i * 10, 5,
			       &liveEmpty, 1.0f);
	}
      else
	{
	  App->renderer->Blit (life, (position.x - 120) + i * 10, 5,
			       &liveEmpty, 1.0f);
	}
