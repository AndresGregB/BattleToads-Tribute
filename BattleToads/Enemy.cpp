...
Enemy::Update ()
{
  ...
  if (active && status != DYING)
      ...
      if (calculateDistancetoPlayer () > 25.0f)
	{
	  ...
	  float module =
	    sqrt ((nextMovement.x * nextMovement.x +
		   nextMovement.y * nextMovement.y));
...
Enemy::Draw ()
{
  if (status == WALK_RIGHT)
    {
      App->renderer->Blit (App->collisions->enemiesControler->graphics,
			   position.x, position.y, &(walk.GetCurrentFrame ()),
			   1.0f, 2);
    }
  ...
  else if (status == DYING)
    {
      App->renderer->Blit (App->collisions->enemiesControler->graphics,
			   ...
			   &(death.GetCurrentFrame ()), 1.0f, 2);
    }
  else if (status == ATTACKING_R)
    {
      App->renderer->Blit (App->collisions->enemiesControler->graphics,
			   position.x, position.y,
			   &(attack.GetCurrentFrame ()), 1.0f, 2);
      if (SDL_GetTicks () - attackTimer > 500 && status != DYING)
	{
	  if (App->
	      collisions->checkAttackHit (attackRH, CoordZ,
					  App->player->playerHitbox,
					  App->player->coordZ))
	    App->player->takeHit ();
	  attacking = false;
	}
  ...
  else if (status == ATTACKING_L)
    {
      App->renderer->Blit (App->collisions->enemiesControler->graphics,
			   position.x, position.y,
			   &(attack.GetCurrentFrame ()), 1.0f, 2, true);
      if (SDL_GetTicks () - attackTimer > 500 && status != DYING)
	{
	  if (App->
	      collisions->checkAttackHit (attackLH, CoordZ,
					  App->player->playerHitbox,
					  App->player->coordZ))
	    App->player->takeHit ();
	  attacking = false;
...
int
Enemy::calculateDistancetoPlayer ()
{
