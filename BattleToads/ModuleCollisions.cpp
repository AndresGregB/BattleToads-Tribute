...
void
ModuleCollisions::drawEnemiesBehind ()
{
...
void
ModuleCollisions::addHitbox (Hitbox * hitBox)
{
  levelLayout.push_back (hitBox);
}

bool
ModuleCollisions::checkIfOnFloor ()
{
  std::list < Hitbox * >::const_iterator it;

  for (it = levelLayout.begin (); it != levelLayout.end (); ++it)
    {
      if ((*it)->checkIntersection (App->player->playerHitbox->hitBox))
	return true;
    }
  return false;
}
...
void
ModuleCollisions::addEnemy (Enemy * enemy)
{
  enemies.push_back (enemy);
}

bool
ModuleCollisions::checkAttackHit (Hitbox * attackHitbox,
				  const float attackerCoordZ,
				  const Hitbox * targetHitbox,
				  const float targetCoordZ)
{
...
void
ModuleCollisions::checkAttackVsEnemies (Hitbox * attackHitbox,
					const float attackerCoordZ)
{
  std::list < Enemy * >::const_iterator it2;
  ...
  for (it2 = enemies.begin (); it2 != enemies.end (); ++it2)
	...
	{
	  if (checkAttackHit
	      (attackHitbox, attackerCoordZ, (*it2)->hitBox, (*it2)->CoordZ))
	    (*it2)->prepareToDie ();
	}
    }
}
